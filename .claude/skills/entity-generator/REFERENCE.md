...
> **Linkable routes.** The site exposes per-slug dynamic routes only — `/experiences/[slug]`, `/hebergements/[slug]`, `/destinations/[slug]`. There is no index page at `/experiences`, `/hebergements`, or `/destinations`. Any `cta.href` pointing at an index route will 404. Canonical hrefs for CTA blocks: `/votre-projet`, `/contact`. Per-entity links generated by the entityList card itself (when the entity has `sections.length > 0`) resolve correctly — only the optional section-level `cta` is at risk.
