# Reference images — Île de Pâques

Each generated image in `public/destination/ile-de-paques/` was produced by feeding the prompt baked into `.claude/skills/destination-generator/gen-images.py` to Gemini 3 Pro Image (Nano Banana Pro) alongside the corresponding reference photograph below.

| Output                              | Reference file                          | Source URL | License |
| ----------------------------------- | --------------------------------------- | ---------- | ------- |
| `hero-1.webp` | `hero-1-ref.png` | à renseigner | à renseigner |
| `hero-2.webp` | `hero-2-ref.png` | à renseigner | à renseigner |
| `hero-3.webp` | `hero-3-ref.png` | à renseigner | à renseigner |
...
Testimonial portraits reuse `hero-1.webp`, `hero-2.webp`, `hero-3.webp` — no separate files.

`split-2.webp` n'a pas de photo de référence : elle a été composée à partir d'un brief textuel documenté (umu pae, four circulaire de pierres volcaniques d'environ 80 cm, jardin de Hanga Roa), sans reprendre aucune photographie existante. Il en va de même pour `xp-rano-raraku-2.webp` (traces de toki dans le tuf de la carrière, affleurement banal et érodé).
