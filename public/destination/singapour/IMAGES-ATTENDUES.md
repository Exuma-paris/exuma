# Images attendues — Singapour

Dossier : `public/destination/singapour/`

**0 sur 25 présentes.** Il manque 25 images.

Déposez chaque fichier dans ce dossier en respectant exactement le nom ci-dessous : c'est ce nom que la page va chercher.

## Carrousel du hero

- [ ] `hero-1.png`
- [ ] `hero-2.png`
- [ ] `hero-3.png`

## Bandeau pleine largeur

- [ ] `full-image.png`

## Section texte et images

- [ ] `split-1.png`
- [ ] `split-2.png`

## Expériences

- [ ] `xp-croisiere-iles-sud.png`
- [ ] `xp-diner-candlenut.png`
- [ ] `xp-jardins-botaniques.png`
- [ ] `xp-paddock-f1-1.png`
- [ ] `xp-paddock-f1-2.png`

## Hébergements

- [ ] `hotel-capella-singapore.png`
- [ ] `hotel-raffles-singapore.png`
- [ ] `hotel-six-senses-duxton.png`

## Bloc bento

- [ ] `bento-adresses.png`
- [ ] `bento-conciergerie.png`
- [ ] `bento-experiences.png`
- [ ] `bento-hebergements.png`
- [ ] `bento-map.png`

## Carte interactive

- [ ] `map-chinatown.png`
- [ ] `map-gardens-by-the-bay.png`
- [ ] `map-katong.png`
- [ ] `map-marina-bay.png`
- [ ] `map-sentosa.png`
- [ ] `map-tanjong-pagar.png`
