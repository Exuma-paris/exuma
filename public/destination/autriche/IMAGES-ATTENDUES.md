# Images attendues — Autriche

Dossier : `public/destination/autriche/`

**0 sur 25 présentes.** Il manque 25 images.

Déposez chaque fichier dans ce dossier en respectant exactement le nom ci-dessous : c'est ce nom que la page va chercher.

## Carrousel du hero

- [ ] `hero-1.png`
- [ ] `hero-2.png`
- [ ] `hero-3.png`

## Bandeau pleine largeur

- [ ] `full-image.png`

## Section texte et images

- [ ] `split-1.png`
- [ ] `split-2.png`

## Expériences

- [ ] `xp-arlberg.png`
- [ ] `xp-lipizzans.png`
- [ ] `xp-musikverein-1.png`
- [ ] `xp-musikverein-2.png`
- [ ] `xp-wachau.png`

## Hébergements

- [ ] `hotel-aurelio-lech.png`
- [ ] `hotel-sacher.png`
- [ ] `hotel-schloss-fuschl.png`

## Bloc bento

- [ ] `bento-adresses.png`
- [ ] `bento-conciergerie.png`
- [ ] `bento-experiences.png`
- [ ] `bento-hebergements.png`
- [ ] `bento-map.png`

## Carte interactive

- [ ] `map-innsbruck.png`
- [ ] `map-lech.png`
- [ ] `map-salzbourg.png`
- [ ] `map-salzkammergut.png`
- [ ] `map-vienne.png`
- [ ] `map-wachau.png`
